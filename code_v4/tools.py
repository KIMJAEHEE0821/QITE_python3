...
def print_state(psi_, nbit, outf):
    for i in range(psi_.shape[0]):
        if np.abs(psi_[i]) > 1e-4:
            outf.write("".join(str(x) for x in Int2Bas(i, 2, nbit)))
            outf.write(f" {np.real(psi_[i]):.12f} {np.imag(psi_[i]):.12f} I \n")

def dump_state(psi_, nbit, fname):
    with open(fname, 'w') as outf:
        for i in np.where(np.abs(psi_) > 1e-4)[0]:
            outf.write(f"{i} {np.real(psi_[i]):.12f} {np.imag(psi_[i]):.12f} \n")

def read_state(nbit, fname):
    psi = np.zeros(2**nbit, dtype=complex)
    V = np.atleast_2d(np.loadtxt(fname))  # Ensure 2D array for single-line cases
    idx = V[:, 0].astype(int)
    psi[idx] = V[:, 1] + 1j * V[:, 2]
    return psi
...
def dump_lanz_vecs(hv, sv, fname):
    with open(fname, 'w') as outf:
        for i in range(len(hv)):
            outf.write(f"{i} {hv[i]:.12f} {sv[i]:.12f} \n")

def dump_lanz_rte(hv, sv, fname):
    with open(fname, 'w') as outf:
        for i in range(len(hv)):
            outf.write(f"{i} {np.real(hv[i]):.8f} {np.imag(hv[i]):.8f} {np.real(sv[i]):.8f} {np.imag(sv[i]):.8f}\n")
